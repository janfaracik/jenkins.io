...
function do_loaddata() {
  ...
  for (var anchors = document.getElementsByTagName('H2'), i = 0; i < anchors.length; i++) {
    ...
    if (r && r.length > 3) {
      ...
      div3.className = 'rate-text';
      let communityReportedIssues = '<span class="related-issues">Community reported issues: ';
      var issues = [];
      ...
      for (j = 0; j < issues.length; j++)
        communityReportedIssues += issues[j].count + '&times;<a href="https://issues.jenkins.io/browse/JENKINS-' + issues[j].id + '">JENKINS-' + issues[j].id + '</a> ';
      communityReportedIssues += '</span>';
      div3.innerHTML = communityReportedIssues;
